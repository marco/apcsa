...
public class BinaryTreeUtilityTest {
	...
	@Test (timeout = 100)
    public void testGetLevelOrderIterator() {
        Iterator<Integer> itr = utility.getLevelOrderIterator(root);
        assertIteratorContains(itr, 5);

        Iterator<Integer> itr2 = utility.getLevelOrderIterator(root2);
        assertIteratorContains(itr2, 5, 7);
        ...
        //                        /
        //                       4
        Iterator<Integer> itr3 = utility.getLevelOrderIterator(root3);
        assertIteratorContains(itr3, 5, 3, 19, 6, 7, 1, 4);
        ...
        //                        /
        //                       4
        Iterator<Integer> itr4 = utility.getLevelOrderIterator(root4);
        assertIteratorContains(itr4, 5, 3, 19, 1, 2, 1, 4);
        ...
        //            / \           /  \
        //           1   4         7    25
        Iterator<Integer> itr5 = utility.getLevelOrderIterator(root5);
        assertIteratorContains(itr5, 5, 3, 19, 1, 4, 7, 25);
    }

    @Test (timeout = 100, expected = NullPointerException.class)
    public void testLevelOrderException(){
        utility.getLevelOrderIterator(null);
    }

    @Test (timeout = 50)
    public void testLevelOrderIsOof1(){
        // largeTree contains over 1 million elements. getLevelOrderIterator
        // should return immediately. Calls to next should also be pretty fast
        // (you shouldn't precompute the traversal).
        Iterator<Integer> itr = utility.getLevelOrderIterator(largeTree);
        assertEquals(new Integer(20), itr.next());
        assertEquals(new Integer(19), itr.next());
        assertEquals(new Integer(19), itr.next());
        assertEquals(new Integer(18), itr.next());
