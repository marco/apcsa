...
public class Shopper {
    /**
     * The seed to use for `rand`.
     */
    private static final int RANDOM_SEED = 1984;

    /**
     * A `Random` to be used for determining whether to upvote or downvote a store.
     */
    private static Random rand = new Random(RANDOM_SEED);

    /**
     * The next `uniqueID` for a generated shopper.
     */
  private static int nextID;

    /**
     * How long this shopper has been waiting.
     */
  private int waitingTime;

    /**
     * This shopper's unique identifier.
     */
  private final int uniqueID;

    /**
     * The groceries that this shopper is purchasing.
     */
  private final List<GroceryInterface> groceries;

    /**
     * The past transactions made by this shopper.
     */
  private final Set<Transaction> pastTransactions;

    /**
     * Whether or not this shopper is in a line.
     */
  private boolean inline;

    /**
     * Whether or not this shopper is irate.
     */
  private boolean irate;

    /**
     * Whether or not `BigBrother` should be notified of a transaction.
     */
  private boolean notifyBigBrother;

    /**
     * The store that this shopper attends.
     */
  ...
  public final boolean isIrate() {
    ...
    /**
     * <p>
     *      Completes the specified {@link Transaction} with this {@link Shopper}. {@link BigBrother}
     *     will stop you if you try to give them a {@link Transaction} that is not theirs or a
     *     {@link Transaction} that they already received.
     * </p>
     * <p>
     *     A satisfied {@link Shopper} will upvote the {@link AbstractGroceryStore} they visited on the
     *     ever growing social networking site {@link SocialNetwork}. If the {@link Shopper} was not
     *     satisfied, they will downvote the {@link AbstractGroceryStore}.
     * </p>
     *
     * @param t
  ...
  public void tick(final Map<AbstractGroceryStore, Set<Transaction>> transactions) {
      final int MAXIMUM_WAITING_TIME = 300;
    notifyBigBrother(transactions);

    if (inline) {
      waitingTime++;
      if (waitingTime >= MAXIMUM_WAITING_TIME) {
        groceries.clear();
        irate = true;
      }
    }
  ...
  private CheckoutLineInterface findShortest(
    ...
    /**
     * Given a {@link List} of {@link AbstractGroceryStore}s, this {@link Shopper} uses the social
     * networking site {@link SocialNetwork} to select where they will go shopping. Once a store has
     * been chosen, the {@link Shopper} will try to find a line they can enter.
     *
     * @param stores
     *            a list of {@link AbstractGroceryStore}s to choose from
     * @return {@code true} if the {@link Shopper} entered a line and {@code false} otherwise.
     */
  public boolean goShopping(final List<AbstractGroceryStore> stores) {
    ...
    if (store != null) {
      return false;
        ...
        final double BEST_STORE_CHANCE = 0.6;

        // In order to have more interesting values for `SocialNetworkWeb`, only go to the "best"
        // store 60% of the time, so that other stores get some ratings too.
        if (rand.nextFloat() < BEST_STORE_CHANCE) {
            AbstractGroceryStore bestStore = SocialNetwork.getSocialNetwork().selectStore(stores);
            store = bestStore;
            return selectLine(bestStore.getLines());
        }

        AbstractGroceryStore randomStore = stores.get(rand.nextInt(stores.size()));
        store = randomStore;
        return selectLine(randomStore.getLines());
  }
